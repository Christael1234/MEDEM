...
(function () {
  ...
  function payrollCardCopyFor() {
    return ['Coming soon', 'Payroll isn’t built on the backend yet.'];
  }
  ...
  function applyDashboardScope(role) {
    ...
    document.querySelectorAll('[data-scope]').forEach((el) => {
      el.style.display = scopes.has(el.dataset.scope) ? '' : 'none';
    });
  ...
  function applyPrincipalDashboardCopy(role) {
    ...
    const feesCopy = dashboardFeesCopy.default;
  ...
  function renderDashboard(role) {
    ...
    document.getElementById('roleEyebrow').textContent = `${today} · ${r.title} view`;
    const name = greetingName();
    document.getElementById('greeting').textContent = `${timeOfDayGreeting()}${name ? ', ' + name : ''}.`;
  ...
  async function patchGreetingWithRealName(role) {
    const user = window.SchoolOS.getUser();
    if (role !== 'student' || !user || user.role !== 'STUDENT') return;
    const greeting = document.getElementById('greeting');
    if (!greeting) return;
    try {
      const p = await window.SchoolOS.api('/portal/student/me');
      greeting.textContent = `${timeOfDayGreeting()}, ${p.firstName}.`;
